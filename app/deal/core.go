...
func (s *dealService) Involve(spec PartSpec) (step.ProcRoot, error) {
	...
	if len(spec.Ctx) > 0 {
		...
		err = s.chnls.TransferCtx(id.Empty(), maps.Values(spec.Ctx), newPID)
		if err != nil {
			s.log.Error("ctx transfer failed",
				slog.Any("reason", err),
				slog.Any("from", id.Empty()),
				slog.Any("pids", spec.Ctx),
				slog.Any("to", newPID),
			)
			return step.ProcRoot{}, err
		}
...
func (s *dealService) takeProc(
	...
	if err != nil {
		s.log.Error("via selection failed",
			slog.Any("reason", err),
			slog.Any("id", viaID),
		)
		return err
...
func (s *dealService) takeProcWith(
	...
	case step.WaitSpec:
		...
		case step.FwdSpec:
			newTerm := step.Subst(term, val.C, val.D.(chnl.ID))
			newProc = step.ProcRoot{
				ID:   id.New(),
				PID:  proc.PID,
				Term: newTerm,
...
func (s *dealService) checkProvider(
	...
	case step.FwdSpec:
		...
		if gotC.Pol() != gotD.Pol() {
			return fmt.Errorf("polarity mismatch: c=%v, d=%v", gotC.Pol(), gotD.Pol())
		}
		return nil
	default:
...
func findState(
	...
	case chnl.ID:
		got, ok := cfg[val]
		if !ok {
			return nil, chnl.ErrDoesNotExist(val)
		}
		gotCh = got
	case sym.ADT:
		got, ok := ctx[val.Name()]
		if !ok {
			return nil, chnl.ErrUnknownName(val)
		}
		gotCh = got
	}
	if gotCh.St == nil {
